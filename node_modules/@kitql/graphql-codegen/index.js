...
const plugin = (schema, documents, config) => {
    ...
    const operationPrefix = visitorPluginCommon.getConfigValue(config.operationPrefix, 'KQL_');
    const jsDocStyle = visitorPluginCommon.getConfigValue(config.jsDocStyle, false);
    // ${jsDocStyle ? `` : ``}
    const clientPath = visitorPluginCommon.getConfigValue(config.clientPath, '../kitQLClient');
    ...
    const out = allAst.definitions
        .map(node => {
        var _a, _b;
        if (node.kind === 'OperationDefinition' && ((_a = node.name) === null || _a === void 0 ? void 0 : _a.value) && node.operation !== 'subscription') {
            const operationName = convertName((_b = node.name) === null || _b === void 0 ? void 0 : _b.value); // AllContinents
            ...
            const operationTypeSuffix = getOperationSuffix(config, node, operationString);
            ...
            lines.push(`	const { subscribe, set, update } = writable${jsDocStyle ? `` : `<RequestResult<${kqltypeQueryAndVariable}>>`}({...defaultStoreValue, operationName, operationType});`);
            lines.push(``);
            ...
            lines.push(``);
            if (node.operation === 'query') {
                lines.push(`			// Cache only in the browser for now. In SSR, we will need session identif to not mix peoples data`);
            ...
            if (node.operation === 'query') {
                ...
                lines.push(`		 */`);
                lines.push(`		// prettier-ignore`);
                lines.push(`		patch(data: ${kqltypeQuery}, variables: ${kqltypeVariable} | null = null, type: PatchType = 'cache-and-store'): void {`);
                lines.push(`			let updatedCacheStore = undefined;`);
                lines.push(`			if(type === 'cache-only' || type === 'cache-and-store') {`);
                lines.push(`				updatedCacheStore = kitQLClient.cacheUpdate<${kqltypeQueryAndVariable}>(operationName, data, { variables });`);
                lines.push(`			}`);
    ...
    prepend.push(`import { defaultStoreValue, RequestStatus, ResponseResultType` +
        `${jsDocStyle
            ? ``
            : `, type PatchType${kqlStoresMutation.length > 0 ? ', type RequestParameters' : ''}, type RequestQueryParameters, type RequestResult`} } from '@kitql/client';`);
