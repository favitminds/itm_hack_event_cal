...
class LocalStorageCache {
    set(operationKey, data) {
        ...
        // Data
        window.localStorage.setItem(fullKey, helper.stry(data, 0));
    }
    get(operationKey, variables = null) {
        //Data
        const v = helper.stry(variables, 0);
    ...
    remove(operationKey, variables = null, allOperationKey = true) {
        ...
        if (rawIndexes) {
            const indexes = JSON.parse(rawIndexes);
            if (allOperationKey) {
                const keysToDelete = indexes.map((v) => helper.stry({ k: operationKey, v }, 0));
...
})(exports.RequestFrom || (exports.RequestFrom = {}));
(function (ResponseResultType) {
    ResponseResultType["Query"] = "Query";
    ResponseResultType["Mutation"] = "Mutation";
})(exports.ResponseResultType || (exports.ResponseResultType = {}));
const defaultStoreValue = {
    status: exports.RequestStatus.NEVER,
    isFetching: false,
    date: new Date().getTime(),
    operationName: '???',
    operationType: exports.ResponseResultType.Query,
    variables: null,
    data: null,
    errors: null,
    from: exports.RequestFrom.NODATA,
    isOutdated: false
};
class KitQLClient {
    constructor(options) {
    ...
    logOperation(from, operation, variables = null) {
        ...
        `${helper.logCyan('From:')} ${helper.logGreen(from)}, ${new Array(7 - from.length + 1).join(' ')}` +
            `${helper.logCyan('Operation:')} ${helper.logGreen(operation)}` +
            `${variables ? `, ${helper.logCyan('Variables:')} ${helper.logGreen(variables)}` : ``}`);
    }
        ...
        if (browser) {
            ...
            if (cachedData !== undefined) {
                ...
                if (xMs < (cacheMs !== null && cacheMs !== void 0 ? cacheMs : this.cacheMs)) {
                    ...
                    return { ...cachedData, from: exports.RequestFrom.CACHE, isOutdated: false };
                }
                return { ...cachedData, from: exports.RequestFrom.CACHE, isOutdated: true };
            }
        }
        return null;
    }
    async request({ skFetch, document, variables, operationName, operationType, browser }) {
        ...
        const dataToReturn = {
            ...
            operationType,
            variables,
            from: exports.RequestFrom.NETWORK,
            data: null,
            errors: null,
            isOutdated: false
        };
        try {
            ...
            if (logStatements.logRawResult) {
                this.log.info(`${helper.logCyan('dataJson:')} ` + `${helper.stry(dataJson, 0)}`);
            }
